...
DEFAULT_TOPIC_CONTENT_CHARS = 1800
DEFAULT_INDEX_MAX_TOKENS = 2500
DEFAULT_TOPIC_MATCH_MAX_TOKENS = 500
...
class TopicEntry:
    ...
    legal_flag_reasons: dict[str, str]
    legal_confidence: str
    commercial_flags: list[str]
    commercial_flag_reasons: dict[str, str]
    commercial_confidence: str
    financial_flags: list[str]
    financial_flag_reasons: dict[str, str]
    financial_confidence: str
    prebid_flags: list[str]
    prebid_flag_reasons: dict[str, str]
    prebid_confidence: str
    created_at: str
...
def clean_topic_name(value: str) -> str:
    ...
    return value[:120] or "Untitled Topic"


def existing_topic_names(index: list[dict[str, Any]]) -> list[str]:
    return [entry["topic_name"] for entry in index]


def next_versioned_name(base_name: str, index: list[dict[str, Any]]) -> str:
    names = set(existing_topic_names(index))
...
def base_topic_name(topic_name: str) -> str:
    return re.sub(r"_v\d+$", "", topic_name).strip()
...
def likely_related_by_name(candidate_name: str, existing_name: str) -> bool:
    candidate = normalize_for_match(base_topic_name(candidate_name))
    existing = normalize_for_match(base_topic_name(existing_name))
...
def candidate_existing_topics(
    raw_topic: dict[str, Any],
    index: list[dict[str, Any]],
    limit: int = 12,
) -> list[dict[str, Any]]:
    topic_name = str(raw_topic.get("topic_name", ""))
    matches = [
        entry for entry in index if likely_related_by_name(topic_name, entry["topic_name"])
    ]
    if not matches:
        matches = index[-limit:]
...
def page_prompt(page: PageUnit, index: list[dict[str, Any]]) -> str:
    known = existing_topic_names(index)
    ...
    return f"""
You are creating a page-level topic index for a mixed document corpus.
...
2. Identify every distinct new topic found on this page.
3. For each topic, write a concise but useful topic description.
4. Store only concise actual page content relevant to that topic. Do not copy long repeated drawing labels, table noise, or repeated words.
...
7. For each topic, set topic_source to:
...
8. Add the tag "image" to tags whenever topic_source is "image" or "mixed", or whenever image_descriptions is not empty.
9. Do not invent content that is not visible or supported.
10. Keep each topic content under {int(os.getenv("INDEX_TOPIC_CONTENT_CHARS", str(DEFAULT_TOPIC_CONTENT_CHARS)))} characters.
11. If a drawing repeats labels like MATCH LINE many times, mention the repetition once instead of copying it.
12. If no meaningful topic exists, return an empty topics array.

Return only valid JSON with this shape:
{{
  "topics": [
    {{
      "topic_name": "short stable topic name",
      "topic_description": "brief definition of what the content is about",
      "topic_source": "text | image | mixed",
      "tags": ["image"],
      "content": "actual content from this page related to the topic",
      "image_descriptions": [
        {{
...
def topic_match_prompt(
    ...
    return f"""
Decide whether the candidate topic should become a version of an existing topic.

Versioning rule:
- If an existing topic has a similar name AND its content is similar to, continuing, or extending the candidate topic, return same_topic=true.
- If only the name is vaguely similar but the content is different, return same_topic=false.
- If same_topic=true, choose the best existing topic and return its base name without any _vN suffix.
- Do not merge content. This decision is only for naming the new topic.

Candidate topic:
{json.dumps(raw_topic, ensure_ascii=False)}
...
{{
  "same_topic": true,
  "matched_base_name": "Topic Name",
  "reason": "short reason"
}}
""".strip()


def decide_topic_name_dry(raw_topic: dict[str, Any], index: list[dict[str, Any]]) -> str:
    base_name = clean_topic_name(str(raw_topic.get("topic_name", "Untitled Topic")))
    for entry in candidate_existing_topics(raw_topic, index):
        if likely_related_by_name(base_name, entry["topic_name"]):
            return next_versioned_name(base_topic_name(entry["topic_name"]), index)
    return next_versioned_name(base_name, index)


def decide_topic_name_openrouter(
    client: OpenAI,
    model: str,
    raw_topic: dict[str, Any],
    index: list[dict[str, Any]],
    ...
    base_name = clean_topic_name(str(raw_topic.get("topic_name", "Untitled Topic")))
    candidates = candidate_existing_topics(raw_topic, index)
    if not candidates:
        return next_versioned_name(base_name, index)
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You compare document-index topics. Return only valid JSON.",
            },
            {"role": "user", "content": topic_match_prompt(raw_topic, candidates)},
        ],
        temperature=0,
        response_format={"type": "json_object"},
        max_tokens=int(os.getenv("OPENROUTER_TOPIC_MATCH_MAX_TOKENS", str(DEFAULT_TOPIC_MATCH_MAX_TOKENS))),
    )
    write_model_hit(
        "topic_match",
        model,
        response,
        {"candidate_topic": base_name},
    )
    message = response.choices[0].message.content or "{}"
    try:
        decision = parse_json_response(message)
    except json.JSONDecodeError:
        save_bad_response("topic_match", message)
        print(
            "Warning: topic-match model response was not valid JSON; "
            "using deterministic topic version fallback."
        )
        return decide_topic_name_dry(raw_topic, index)
    if bool(decision.get("same_topic")):
        matched_base = clean_topic_name(str(decision.get("matched_base_name") or base_name))
        return next_versioned_name(base_topic_name(matched_base), index)
    return next_versioned_name(base_name, index)
...
def dry_extract_page(page: PageUnit) -> dict[str, Any]:
    ...
    topic_name = clean_topic_name(heading[:80])
    image_descriptions = []
    if page.image_path:
        topic_source = "mixed" if text else "image"
        tags = ["image"]
        ...
        "topics": [
            {
                ...
                "topic_description": f"Content found on page {page.page_no} of {page.document_name}.",
...
def extract_page_with_retries(
    ...
    if isinstance(last_error, json.JSONDecodeError):
        ...
        print(
            ...
            f"{max_attempts} attempt(s); using a placeholder topic."
        )
...
def normalize_topic_source(raw_topic: dict[str, Any]) -> str:
    value = str(raw_topic.get("topic_source", "text")).strip().lower()
    if value not in {"text", "image", "mixed"}:
        value = "image" if raw_topic.get("image_descriptions") else "text"
    return value


def normalize_tags(raw_topic: dict[str, Any], topic_source: str) -> list[str]:
    raw_tags = raw_topic.get("tags") or []
    ...
    if topic_source in {"image", "mixed"} or raw_topic.get("image_descriptions"):
        tags.add("image")
    return sorted(tags)
...
def append_topics(
    ...
    added = 0
    for raw_topic in extracted.get("topics", []):
        topic_name = (
            decide_topic_name_dry(raw_topic, index)
            if dry_run or client is None
            else decide_topic_name_openrouter(client, model, raw_topic, index)
        )
        topic_source = normalize_topic_source(raw_topic)
        entry = TopicEntry(
            topic_name=topic_name,
            topic_description=str(raw_topic.get("topic_description", "")).strip(),
            document_id=page.document_id,
            document_name=page.document_name,
            page_no=page.page_no,
            content=str(raw_topic.get("content", "")).strip(),
            source_type="page",
            topic_source=topic_source,
            tags=normalize_tags(raw_topic, topic_source),
            image_descriptions=raw_topic.get("image_descriptions") or [],
            legal_flags=[],
            legal_flag_reasons={},
            legal_confidence="low",
            commercial_flags=[],
            commercial_flag_reasons={},
            commercial_confidence="low",
            financial_flags=[],
            financial_flag_reasons={},
            financial_confidence="low",
            prebid_flags=[],
            prebid_flag_reasons={},
            prebid_confidence="low",
            created_at=datetime.now(timezone.utc).isoformat(),
        )
        entry_dict = asdict(entry)
        flags, reasons, confidence = detect_legal_flags(entry_dict)
        entry_dict["legal_flags"] = flags
        entry_dict["legal_flag_reasons"] = reasons
        entry_dict["legal_confidence"] = confidence
        commercial_flags, commercial_reasons, commercial_confidence = detect_commercial_flags(entry_dict)
        entry_dict["commercial_flags"] = commercial_flags
        entry_dict["commercial_flag_reasons"] = commercial_reasons
        entry_dict["commercial_confidence"] = commercial_confidence
        financial_flags, financial_reasons, financial_confidence = detect_financial_flags(entry_dict)
        entry_dict["financial_flags"] = financial_flags
        entry_dict["financial_flag_reasons"] = financial_reasons
        entry_dict["financial_confidence"] = financial_confidence
        prebid_flags, prebid_reasons, prebid_confidence = detect_prebid_flags(entry_dict)
        entry_dict["prebid_flags"] = prebid_flags
        entry_dict["prebid_flag_reasons"] = prebid_reasons
        entry_dict["prebid_confidence"] = prebid_confidence
...
def index_documents(dry_run: bool, limit_pages: int | None, reset: bool) -> None:
    ...
    index = [] if reset else read_json(TOPIC_INDEX_PATH, [])
    ...
    for record in manifest:
        for page in build_page_units(record):
            ...
            if limit_pages is not None and processed_pages >= limit_pages:
                write_json(TOPIC_INDEX_PATH, index)
                print(f"Page limit reached. Wrote {len(index)} topic entries.")
                return
    ...
    write_pipeline_progress(
        {
            ...
            "message": f"Index complete. Wrote {len(index)} topic entries.",
            "total_pages": total_pages,
            "indexed_pages": len(indexed_pages),
            "topic_count": len(index),
            "failed_pages": len(failed_pages),
    ...
    print(f"Index complete. Wrote {len(index)} topic entries to {TOPIC_INDEX_PATH}.")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Create a page-by-page topic index.")
    ...
    parser.add_argument("--reset", action="store_true", help="Start a fresh topic index.")
    return parser
