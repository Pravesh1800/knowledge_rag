...
from storage import sync_documents
...
def save_manifest(records: list[dict]) -> None:
    if MANIFEST_PATH.exists():
        ...
        backup_path = MANIFEST_PATH.with_name(f"manifest.{timestamp}.bak.json")
        shutil.copy2(MANIFEST_PATH, backup_path)
    MANIFEST_PATH.write_text(
        json.dumps(records, indent=2, ensure_ascii=False),
        encoding="utf-8",
    )
    sync_documents(records)
...
def add_path(raw_path: str) -> None:
    ...
    for file_path in iter_files(Path(raw_path)):
        ...
        replaced_records = [
            ...
            if item.get("source_path") == source_path and item.get("sha256") != record.sha256
        ]
        if replaced_records:
            replaced_ids = {item.get("document_id") for item in replaced_records}
            manifest = [
                item
                for item in manifest
                if item.get("document_id") not in replaced_ids
            ]
            for item in replaced_records:
                existing_hashes.discard(str(item.get("sha256", "")))
            print(f"Replaced changed source file: {file_path}")
