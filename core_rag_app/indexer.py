...
from cache import cache_enabled, cache_path, read_cache, stable_hash, write_cache
from embeddings import build_card_embeddings
from entity_claims import build_entity_claim_index
from llm_config import create_chat_client, get_model
from schema import CARD_INDEX_SCHEMA_VERSION, card_stable_id, read_card_index, write_card_index
from storage import (
    clear_cards,
    ensure_postgres_ready,
    prune_cards_to_documents,
    read_cards,
    replace_page_cards,
    sync_cards,
)
...
PDF_PAGE_RENDER_CACHE_VERSION = "pdf_page_render.v1.0"
PAGE_TEXT_CACHE_VERSION = "page_text.v1.0"
CARD_EXTRACTION_PROMPT_VERSION = "card_extraction_prompt.v1.0"
...
def ensure_card_ids(index: list[dict[str, Any]]) -> bool:
    changed = False
    for entry in index:
        if entry.get("schema_version") != CARD_INDEX_SCHEMA_VERSION:
            entry["schema_version"] = CARD_INDEX_SCHEMA_VERSION
            changed = True
        if not entry.get("card_id"):
            entry["card_id"] = card_stable_id(
                str(entry.get("document_id", "")),
                int(entry.get("page_no") or 0),
...
def persist_card_index(index: list[dict[str, Any]]) -> None:
    try:
        write_card_index(CARD_INDEX_PATH, index)
    except OSError as exc:
        print(f"Warning: PostgreSQL index is saved, but JSON export failed: {exc}")
...
def render_pdf_page(page: fitz.Page, output_path: Path, record: dict[str, Any], page_no: int) -> None:
    output_path.parent.mkdir(parents=True, exist_ok=True)
    key = {
        "version": PDF_PAGE_RENDER_CACHE_VERSION,
        "document_id": record.get("document_id", ""),
        "page_no": page_no,
        "matrix": "2x2",
        "alpha": False,
    }
    cached_path = cache_path("pdf_page_render", key, ".png")
...
def extract_pdf_page_text(page: fitz.Page, record: dict[str, Any], page_no: int) -> str:
    key = {
        "version": PAGE_TEXT_CACHE_VERSION,
        "document_id": record.get("document_id", ""),
        "page_no": page_no,
        "extractor": "fitz.get_text.text",
    }
    cached = read_cache("page_text", key)
    if cached is not None:
        return str(cached.get("value", ""))
    text = page.get_text("text").strip()
    write_cache("page_text", key, text)
    return text
...
def build_pdf_pages(record: dict[str, Any]) -> Iterable[PageUnit]:
    ...
    for page_index, page in enumerate(document, start=1):
        ...
        render_pdf_page(page, image_path, record, page_index)
        yield PageUnit(
            document_id=record["document_id"],
            document_name=record["original_name"],
            page_no=page_index,
            text=extract_pdf_page_text(page, record, page_index),
...
def openrouter_extract_page(
    ...
    prompt = page_prompt(page, index)
    cache_key = {
        "version": CARD_EXTRACTION_PROMPT_VERSION,
    ...
    cached = read_cache("card_extraction", cache_key)
    if cached is not None:
    ...
    try:
        ...
        write_cache("card_extraction", cache_key, result)
        return result
    except json.JSONDecodeError:
        ...
        image_descriptions.append(
            {
                ...
                "description": (
                    ...
                    "Set the active provider API key to generate a detailed model image description."
                ),
            }
        )
    else:
...
def append_cards(
    ...
    for raw_card in extracted.get("cards", []):
        ...
        entry = CardEntry(
            schema_version=CARD_INDEX_SCHEMA_VERSION,
            card_id=card_stable_id(page.document_id, page.page_no, card_name),
            card_name=card_name,
...
def index_documents(
    ...
    build_entity_claims: bool = False,
) -> None:
    load_dotenv()
    ensure_postgres_ready()
    ...
    if reset:
        ...
        index = read_cards()
        if not index and CARD_INDEX_PATH.exists():
            index = read_card_index(CARD_INDEX_PATH, persist_migration=True)
            if index:
                sync_cards(index)
                print(f"Migrated {len(index)} existing JSON card entries into PostgreSQL.")
    if index and ensure_card_ids(index):
        sync_cards(index)
        persist_card_index(index)

    if not manifest:
        raise SystemExit("No ingested documents found. Run: python ingest.py add <path>")

    valid_document_ids = {str(record.get("document_id", "")) for record in manifest}
    if not reset:
        removed_count = prune_cards_to_documents(valid_document_ids)
        retained_index = [
            entry
            for entry in index
            if str(entry.get("document_id", "")) in valid_document_ids
        ]
        if removed_count or len(retained_index) != len(index):
            index = retained_index
            persist_card_index(index)
            print(f"Pruned {removed_count} card entry(s) for removed or replaced documents.")
    ...
    failed_pages = [
        issue
        for issue in read_json(FAILED_PAGES_PATH, [])
        if str(issue.get("document_id", "")) in valid_document_ids
    ]
    placeholder_pages = [
        issue
        for issue in read_json(PLACEHOLDER_PAGES_PATH, [])
        if str(issue.get("document_id", "")) in valid_document_ids
    ]
    write_json(FAILED_PAGES_PATH, failed_pages)
    write_json(PLACEHOLDER_PAGES_PATH, placeholder_pages)
    ...
    for record in manifest:
        for page in build_page_units(record):
            ...
            if limit_pages is not None and processed_pages >= limit_pages:
                persist_card_index(index)
                if build_embeddings and not dry_run:
                    build_card_embeddings(index)
                if build_entity_claims:
                    build_entity_claim_index(index, dry_run=dry_run)
            ...
            page_cards = [
                entry
                for entry in index
                if str(entry.get("document_id", "")) == page.document_id
                and int(entry.get("page_no") or 0) == page.page_no
            ]
            replace_page_cards(
                page_cards,
                document_id=page.document_id,
    ...
    persist_card_index(index)
    if build_embeddings and not dry_run:
        print("Building card embeddings...")
        build_card_embeddings(index)
    if build_entity_claims:
        print("Building entity/claim anchors...")
        build_entity_claim_index(index, dry_run=dry_run)
    print(f"Index complete. Wrote {len(index)} card entries to PostgreSQL. JSON export: {CARD_INDEX_PATH}.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--build-embeddings", action="store_true", help="Build or refresh the hybrid retrieval embedding index.")
    parser.add_argument("--build-entity-claims", action="store_true", help="Build or refresh typed entity and claim anchors.")
    return parser
...
def main() -> None:
    ...
    index_documents(
        ...
        build_entity_claims=args.build_entity_claims,
    )
